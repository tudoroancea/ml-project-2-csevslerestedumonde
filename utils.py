...
def dice_coeff(pred: torch.Tensor, target: torch.Tensor, eps=1.0):
    """
    Computes the Sorensen-Dice coefficient, also known as the F1 score.
    Args:
        pred: A tensor of shape [N, 1, H, W] where N is the batch size.
            Each value represents the probability that the corresponding pixel is
            a road.
        target: A tensor of shape [N, 1, H, W] where N is the batch size.
            Each value is 1 for pixels that are roads, and 0 for the rest.
        eps: added to the denominator for numerical stability
    Returns:
        The mean F1 score for the batch.
    """
    assert pred.shape == target.shape and pred.shape[1:] == (1, 400, 400)
    return torch.mean(
        (eps + 2 * torch.sum(pred * target, dim=0))
        / (eps + torch.sum(pred**2, dim=0) + torch.sum(target**2, dim=0))
    )


def jaccard_index(pred: torch.Tensor, target: torch.Tensor, eps=1.0):
    """
...
def tpr_score(prediction: torch.Tensor, target: torch.Tensor) -> float:
    """
    Computes the true positive rate.
    Args:
        target: A tensor of shape [N, 1, H, W] where N is the batch size.
            Each value is 1 for pixels that are roads, and 0 for the rest.
        prediction: A tensor of shape [N, 1, H, W] where N is the batch size.
            Each value represents the probability that the corresponding pixel is
            a road.
    Returns:
        The true positive rate.
    """
    return (torch.sum(target * prediction) / torch.sum(target)).item()


def fpr_score(prediction: torch.Tensor, target: torch.Tensor) -> float:
    """
    ...
    """
    return (torch.sum((1 - target) * prediction) / torch.sum(1 - target)).item()
